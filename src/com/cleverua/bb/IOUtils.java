...
public class IOUtils {
    ...
    /**
     * If target file exists, then the method does nothing.
     * 
     * @param is InputStream to read data to save
     * @param url - path to output file
     * @throws IOException
     */
    public static void saveInputStreamDataToFileIfNotExists(String url, InputStream is) 
            throws IOException {
    ...
    /**
     * Returns the full name of the file from the given url.
     */
    public static String getFileNameFromPath(String url) {
        if (url != null) {
            return url.substring(url.lastIndexOf(Characters.SOLIDUS) + 1);
        }
        return null;
    }
    
    /**
     * Returns the path of the file (including '\') from the given url
     */
    public static String getFilePath(String url) {
        if (url != null) {
            return url.substring(0, url.lastIndexOf(Characters.SOLIDUS) + 1);
        }
        return null;
    }
    
    /**
     * Returns only the name of the file (without extension) from the given url.
     */
    public static String getFileNameWithoutExtension(String url) {
        if (url != null) {
            return url.substring(url.lastIndexOf(Characters.SOLIDUS) + 1, url.lastIndexOf('.'));
        }
        return null;
    }
    
    /**
     * Returns the extension of the file (including '.') from the given url.
     */
    ...
    /**
     * Returns the given url name with the "-increment" ("-1", "-2", "-15" etc) string 
     * appended to the name of the file.
     */
    public static String getIncrementedFileName(String url, int increment) {
        StringBuffer newUrl = new StringBuffer();
        newUrl.append(IOUtils.getFilePath(url));
        newUrl.append(IOUtils.getFileNameWithoutExtension(url));
        newUrl.append('-').append(increment);
        newUrl.append(IOUtils.getFileExtension(url));
